...
document.addEventListener("turbolinks:load", () => {
  ...
  const venueNameHidden = document.getElementById("venue_name_hidden");
  const venueLocationHidden = document.getElementById("venue_location_hidden");
    ...
    try {
      ...
      if (data.length > 0) {
        data.forEach(([dateInfo]) => {
          ...
          option.dataset.venueName = dateInfo.venue_name;
          option.dataset.venueLocation = dateInfo.venue_location;
  ...
  dateSelect.addEventListener("change", (event) => {
    const selectedDate = event.target.options[event.target.selectedIndex];
    venueLocationHidden.value = selectedDate.dataset.venueLocation;
    venueNameHidden.value = selectedDate.dataset.venueName;
  });
});
